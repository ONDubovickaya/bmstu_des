...
void graph_to_dot_result(const std::vector<Edge>& edges, const std::string& filename, int startNode, int finishNode, const std::vector<int>& nodesWay) {
    std::ofstream dotFile(filename);
    ...
    for (int i = 1; i < nodesWay.size(); i++) {
        int currentNode = nodesWay[i-1];
        coloredWay[currentNode] = nodesWay[i];
    }
    ...
    for (const auto& edge : edges) {
        ...
        if (coloredWay.find(end) != coloredWay.end() && coloredWay[end] == start) {
